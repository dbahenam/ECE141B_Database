...
//};
  ...
  static std::map<Operators, Comparitor> comparitors {
	...
	{Operators::notequal_op, Compare::equals},
	{Operators::lt_op, Compare::lessThan},
	{Operators::lt_op, Compare::greaterThan}
  };
  ...
  bool Filters::matches(KeyValues &aList) const {
	...
	if (expressions.size() > 1) {
		for (auto theIter = expressions.begin(); theIter != prev(expressions.end(), 1); ++theIter) {
			 auto theNext = std::next(theIter, 1);
			 Expression exp1 = *(*theIter);
			 Expression exp2 = *(*theNext);
			
			if (exp1.logic == Logical::and_op) {
			  //matches = andIsTrue(exp1, exp2, aList);
			}
			if (exp1.logic == Logical::or_op)  {
			 // matches = orIsTrue(exp1, exp2, aList);
			}
		}
		return matches;
	}
